...
For demos that need remote or unattended verification, prefer two separate entrypoints:

- `validate.sh` for idempotent post-deploy validation. It should not run setup. It should verify the live deployment is healthy and that the demo behavior is correct.
- `test_runner.sh` for a full run from setup through validation. It should call `setup.sh`, then `validate.sh`, and capture useful artifacts such as logs, pod lists, and events under `artifacts/`.

Document both scripts in the demo `README.md`, and mention them in `demo_setup.md` or `demo_validation.md` as appropriate.

Validation scripts should assert behavior, not just command success. For secrets demos, that means failing if an expected secret is empty, if a protected workload exposes sensitive mounted files, or if the defended workload cannot prove that it retrieved secrets through the intended identity path.

## Commit Readiness

Before a demo update is committed, verify the staged content is suitable for the repo, not just technically working in the lab.
...
- Remove attribution lines from code and docs, including `Authored by`, `Generated by`, `Created by`, `Co-authored-by`, and AI/tool signatures, unless the user explicitly asks for them.
- Treat demo placeholder credentials as acceptable only when they are fake, documented, and part of the teaching flow.
- Call out any intentionally committed binary assets, such as Helm chart archives, and make sure the docs explain where they came from.
...
For SWA and Terraform-based demos, do not commit `.terraform/`, Terraform state, downloaded providers, or generated registration files. Commit Terraform source files and setup scripts. Follow the repo's `.gitignore` policy for `.terraform.lock.hcl` unless the demo documentation explicitly states that the lock file is required.
